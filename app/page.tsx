...
export default function Home(){
  ...
  const canGoPrevious=TEST_ALL_LEVELS||level>0,canGoNext=level<levels.length-1&&(TEST_ALL_LEVELS||level+1<unlocked),remaining=Math.max(0,challengeFor(level)-time);
  return <main className={`app-shell theme-${theme}`}><section className="game-card">
    <header className="mobile-top"><button className="mobile-brand" onClick={()=>setShopOpen(true)}>KELUAR<span>.</span><small>◆ {tokens}</small></button><div className="mobile-level"><button onClick={goPrevious} disabled={!canGoPrevious||paused} aria-label="Level sebelumnya">‹</button><span><small>LEVEL</small>{String(level+1).padStart(2,"0")}<em>{"★".repeat(bestStars[level]??0)}{"☆".repeat(3-(bestStars[level]??0))}</em></span><button onClick={goNext} disabled={!canGoNext||paused} aria-label="Level berikutnya">›</button></div><div className="mobile-timer"><button className={`pause-btn ${paused?"paused":""}`} onClick={togglePause} aria-label={paused?"Lanjutkan permainan":"Jeda permainan"}>{paused?"▶":<span className="pause-mark"/>}</button><div className={`mobile-stats ${challengeMode&&remaining<=10000?"time-danger":""}`}><strong>{fmt(challengeMode?remaining:time)}</strong><small>{paused?"DIJEDA":challengeMode?`TANTANGAN · ${pieces.length} SISA`:`${pieces.length} SISA`}</small></div></div></header>
    <div className={`block-board ${blocked?"board-blocked":""} ${showGrid?"":"grid-off"}`}>
      ...
      {paused&&<div className="pause-cover"><button onClick={togglePause}><b>▶</b><strong>Permainan dijeda</strong><small>Ketuk untuk lanjut</small></button></div>}
    </div>
    <nav className="mobile-bottom"><button onClick={undo} disabled={paused||!history.length}><b>↶</b>Urungkan</button><button onClick={()=>load(level)} disabled={paused}><b>↻</b>Ulangi</button><button onClick={useHint} disabled={paused}><b>◆</b>Petunjuk</button><button onClick={()=>setModeOpen(true)} className={challengeMode?"active-mode":""}><b>⏱</b>Mode</button><button onClick={()=>setHelp(true)}><b>?</b>Aturan</button></nav>
  </section>
