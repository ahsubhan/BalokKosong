...
export default function Home(){
  ...
  useEffect(()=>{if(won||timedOut||onboarding)return;started.current=performance.now();setTime(0);const limit=challengeFor(level);const t=setInterval(()=>{const elapsed=performance.now()-started.current;if(challengeMode&&elapsed>=limit){setTime(limit);setTimedOut(true);clearInterval(t);return}setTime(elapsed)},100);return()=>clearInterval(t)},[level,round,won,timedOut,challengeMode,onboarding]);
